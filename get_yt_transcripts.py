...
TARGET_CHANNEL_ID = "UCEAwYFRFUuekaEmKz4oiB_A"
...
def get_all_video_ids_in_playlist(youtube, playlist_id):
    ...
    while retries > 0:
        try:
            ...
            print(f"API Error fetching playlist items (Page Token: {next_page_token}): {e}")
            retries -= 1
            if retries > 0:
                wait_time = 2 ** (max_retries - retries) # Exponential backoff
                print(f"Retrying playlist fetch in {wait_time}s... ({retries} attempts left)")
    ...
    for i in range(0, len(video_ids), 50):
        ...
        while retries > 0:
            try:
                ...
                if retries > 0:
                    wait_time = 2 ** (max_retries - retries) # Exponential backoff
                    print(f"Retrying metadata fetch in {wait_time}s... ({retries} attempts left)")
                    time.sleep(wait_time)
                else:
                    ...
                    for vid in batch_ids:
                        if vid not in metadata_dict:
                             metadata_dict[vid] = {"error": "Metadata fetch failed after retries"}
            except Exception as e:
                print(f"Unexpected error fetching metadata batch {i}: {e}")
                # Mark videos in this failed batch with an error
                for vid in batch_ids:
                    if vid not in metadata_dict:
                         metadata_dict[vid] = {"error": f"Unexpected metadata fetch error: {e}"}
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(f"Transcript in {languages} not found for {video_id}. Trying any available language.")
                available_transcripts = list(transcript_list)
                if available_transcripts:
                    transcript = available_transcripts[0]
                    found_lang = transcript.language
                    transcript_type = 'manual' if not transcript.is_generated else 'generated'
                    print(f"Found fallback {transcript_type} transcript in language: {found_lang} for {video_id}")
                else:
        ...
        if not fetched_transcript_list:
            print(f"Fetched transcript list is empty for {video_id} (Language: {found_lang})")
        ...
        for item in fetched_transcript_list:
            try:
                ...
                if text and isinstance(text, str):
                    cleaned_text = ' '.join(text.split())  # Remove extra whitespace/newlines
        ...
        if not full_transcript_parts:
            print(f"No valid transcript text extracted for {video_id} (Language: {found_lang})")
            return None, found_lang
...
if __name__ == "__main__":
    ...
    if not DEVELOPER_KEY:
        ...
        youtube_service = get_youtube_service() # get_youtube_service now handles the None check
        ...
        if youtube_service:
            ...
            if upload_playlist_id:
                ...
                if total_videos > 0:
                    ...
                    print(f"\nProcessing videos and writing to output files (limit ~{MAX_WORDS_PER_FILE:,} words/file)...") # Formatted limit
                    ...
                    try: # Wrap the loop in try...finally to ensure the last file is closed/renamed
                        for i, video_id in enumerate(video_ids):
                            current_video_number = i + 1 # 1-based index for user display/naming
                            ...
                            if "error" in metadata:
                                print(f"  Skipping transcript fetch for {video_id} due to metadata fetch error: {metadata['error']}")
                                transcript_lang = f"Skipped ({metadata['error']})"
                                videos_with_issues += 1
                            else:
                                # Fetch transcript for this video
                                transcript_text, transcript_lang = get_transcript_for_video(video_id, PREFERRED_LANGUAGES)
                                if transcript_text: # Checks for non-empty string
                                    successful_transcripts += 1
                                    print(f"  Transcript fetched ({transcript_lang})")
                                elif transcript_text == "": # Explicitly check for empty string
                                    print(f"  Transcript fetched ({transcript_lang}), but content is empty.")
                                    videos_with_issues += 1
                                else: # transcript_text must be None
                                    # Reason already printed by get_transcript_for_video
                                    print(f"  Transcript fetch failed or unavailable ({transcript_lang}).")
                                    videos_with_issues += 1

                            # --- Prepare entry text and estimate word count ---
                            video_entry_text = format_video_entry(
                                current_video_number, video_id, metadata, transcript_text, transcript_lang
                            )
                            ...
                            needs_new_file = current_file_handle is None or \
                                             (current_word_count + entry_word_count > MAX_WORDS_PER_FILE and current_word_count > 0)
                            ...
                            if needs_new_file:
                                if current_file_handle is not None:
                                    ...
                                    final_filepath = os.path.join(output_dir, final_filename_base)
                                    try:
                                        os.rename(current_filename_temp, final_filepath)
                                        print(f"Saved: {final_filepath}")
                                        output_filenames.append(final_filepath)
                                    except OSError as e:
                                        print(f"Error renaming {current_filename_temp} to {final_filepath}: {e}")
                                        output_filenames.append(current_filename_temp + " (rename failed)")
                                    current_file_handle = None
                                ...
                                current_filename_temp = os.path.join(output_dir, temp_filename_base)
                                ...
                                try:
                                    ...
                                    current_file_handle = open(current_filename_temp, "w", encoding="utf-8")
                                    ...
                                    current_word_count = len(header.split()) # Reset word count for the new file
                                except IOError as e:
                            ...
                            if current_file_handle:
                                try:
                                    current_file_handle.write(video_entry_text + "\n") # Add extra newline for spacing
                                    current_word_count += entry_word_count
                                except IOError as e:
                                    print(f"Error writing video {current_video_number} to {current_filename_temp}: {e}")
                                    # Attempt to close and finalize the file partially
                                    current_file_handle.close()
                                    last_video_number_in_file = current_video_number - 1 # Video that failed wasn't fully written
                                    ...
                                    partial_filepath = os.path.join(output_dir, partial_filename_base)
                                    try:
                                        os.rename(current_filename_temp, partial_filepath)
                                        print(f"Saved partial file: {partial_filepath}")
                                        output_filenames.append(partial_filepath)
                                    except OSError as rename_e:
                                         print(f"Error renaming partial file {current_filename_temp}: {rename_e}")
                                         output_filenames.append(current_filename_temp + " (partial, rename failed)")
                                    current_file_handle = None
                        ...
                        if current_file_handle is not None:
                            ...
                            last_video_number_in_file = current_video_number # Use the last processed number
                            final_filename_base = f"video-{get_ordinal(file_start_video_number)}-{get_ordinal(last_video_number_in_file)}-transcripts.txt"
                            final_filepath = os.path.join(output_dir, final_filename_base)
                            try:
                                os.rename(current_filename_temp, final_filepath)
                                ...
                                print(f"Error renaming final file {current_filename_temp} to {final_filepath}: {e}")
                                output_filenames.append(current_filename_temp + " (rename failed)")
                ...
                print(f"Could not find the uploads playlist for channel {TARGET_CHANNEL_ID}. Cannot proceed.")
        else:
